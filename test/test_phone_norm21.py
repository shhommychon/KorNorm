...
class TestPhoneNorm21(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    #        (연구개음화·양순음화 등 수의적 동화가 과발동하지 않는지 검증하는 네거티브 조항)
    # ================================================================================
    def test_norm21_words(self):
        """개별 단어 검증: 제21항 본항 (그 외 자음 동화 불인정)"""
        cases = {
            "감기": "감기",       # x[강기]
            "옷감": "옫깜",       # x[옥깜]
            "있고": "읻꼬",       # x[익꼬]
    ...
    def test_norm21_sentences(self):
        """통합 엔진 검증: 제21항 본항 포함 문장"""
        sentence = self._strip_punctuation("감기에 걸린 젖먹이 동생이 꽃밭 가운데 꽃길에서 놀고 있고, 어머니는 옷감을 골라 두고 문법 책을 집어 들었다.")
        expected = "감기에 걸린 전머기 동생이 꼳빧 가운데 꼳끼레서 놀고 읻꼬 어머니는 옫까믈 골라 두고 문뻡 채글 지버 드럳따"
