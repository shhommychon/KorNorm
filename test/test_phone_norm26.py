...
from kornorm.phonology.chapter6 import norm24, norm25, norm26
from kornorm.utils.jamo import join_jamos

class TestPhoneNorm26(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _word_pipeline(self, word: str) -> str:
        """
        단어 검증용 축약 파이프라인: 엔진 순서(사전 발음 선적용 -> norm20_p -> norm26 -> norm5_p2 -> norm10_p -> norm24 -> norm25 -> LUT -> norm13 -> norm14)를 재현합니다.
    ...
    # ================================================================================
    # 제26항 한자어에서 ㄹ 받침 뒤에 연결되는 ㄷ, ㅅ, ㅈ은 된소리로 발음
    # ================================================================================
    def test_norm26_words(self):
        """개별 단어 검증: 제26항 본항 (한자어 ㄹ 받침 뒤 경음화)"""
        cases = {
            "갈등": "갈뜽",
            "발동": "발똥",
            "절도": "절또",
    ...
    def test_norm26_sentences(self):
        """통합 엔진 검증: 제26항 본항 포함 문장"""
        sentence = self._strip_punctuation("말살 정책에 반대하던 그는 갈등과 갈증을 누르고 일시에 불세출 발명을 내놓아 발전을 오래 이끌었다.")
        expected = "말쌀 정채게 반대하던 그는 갈뜽과 갈쯩을 누르고 일씨에 불쎄출 발명을 내노아 발쩌늘 오래 이끄럳따"
    ...
    def test_norm26_proviso_sentences(self):
        """통합 엔진 검증: 제26항 다만 포함 문장"""
        sentence = self._strip_punctuation("허허실실 전략을 쓰던 장수는 절절하게 호소했다.")
        expected = "허허실실 절랴글 쓰던 장수는 절절하게 호소핻따"
        actual = apply_phonology(sentence, output_format="hangul")
        g2pk_res = self.g2pk(sentence)
