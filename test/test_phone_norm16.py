...
from kornorm.phonology.chapter4 import norm13, norm16
from kornorm.utils.jamo import join_jamos

class TestPhoneNorm16(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    # ================================================================================
    # 제16항 한글 자모 이름의 받침 연음
    #        ('ㄷ, ㅈ, ㅊ, ㅋ, ㅌ, ㅍ, ㅎ'의 받침은 [ㅅ, ㄱ, ㅂ]으로 특별히 발음)
    # ================================================================================
    def test_norm16_words(self):
        """개별 단어 검증: 제16항 (한글 자모 이름의 예외 연음)"""
        cases = {
            "디귿이": "디그시",
            "디귿을": "디그슬",
            "디귿에": "디그세",
    ...
    def test_norm16_sentences(self):
        """통합 엔진 검증: 제16항 포함 문장"""
        sentence = self._strip_punctuation("받아쓰기 시간에 디귿이 어렵다던 아이는 지읒을 겨우 쓰고, 치읓에 키읔이 헷갈린다며 티읕을 건너뛰고 피읖에 히읗을 적었다.")
        expected = "바다쓰기 시가네 디그시 어렵따던 아이는 지으슬 겨우 쓰고 치으세 키으기 헫깔린다며 티으슬 건너뛰고 피으베 히으슬 저걷따"
