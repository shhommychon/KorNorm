...
from kornorm.phonology.chapter5 import norm17, norm17_a
from kornorm.utils.jamo import join_jamos

class TestPhoneNorm17(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    # ================================================================================
    # 제17항 받침 ㄷ, ㅌ(ㄾ) 뒤에 조사/접미사의 모음 ㅣ 결합
    #        -> [ㅈ, ㅊ]으로 바꾸어서 뒤 음절 첫소리로 연음 (구개음화)
    # ================================================================================
    def test_norm17_words(self):
        """개별 단어 검증: 제17항 본항 (ㄷ, ㅌ(ㄾ)의 구개음화)"""
        cases = {
            "곧이듣다": "고지듣따",
            "굳이": "구지",
            "미닫이": "미다지",
    ...
    def test_norm17_sentences(self):
        """통합 엔진 검증: 제17항 본항 포함 문장"""
        sentence = self._strip_punctuation("허튼 말을 곧이듣다 굳이 미닫이 앞에 서서, 땀받이 수건을 두르고 밭이 넓다며 벼훑이 손질을 했다.")
        expected = "허튼 마를 고지듣따 구지 미다지 아페 서서 땀바지 수거늘 두르고 바치 널따며 벼훌치 손지를 핻따"
    ...
    # ================================================================================
    # 제17항 붙임 ㄷ 뒤에 접미사 '히'가 결합되어 '티'를 이루는 것 -> [치]
    # ================================================================================
    def test_norm17_addendum_words(self):
        """개별 단어 검증: 제17항 붙임 (ㄷ + 접미사 '히' -> [치])"""
        cases = {
            "굳히다": "구치다",
            "닫히다": "다치다",
            "묻히다": "무치다",
    ...
    def test_norm17_addendum_sentences(self):
        """통합 엔진 검증: 제17항 붙임 포함 문장"""
        sentence = self._strip_punctuation("승부를 굳히다 말고 일어난 그는, 문이 닫히다 말고 열린 틈으로 흙에 묻히다시피 한 신발을 보았다.")
        expected = "승부를 구치다 말고 이러난 그는 무니 다치다 말고 열린 트므로 흘게 무치다시피 한 신바를 보앋따"
        actual = apply_phonology(sentence, output_format="hangul")
        g2pk_res = self.g2pk(sentence)
