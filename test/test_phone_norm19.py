...
class TestPhoneNorm19(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    # ================================================================================
    # 제19항 받침 ㅁ, ㅇ 뒤에 연결되는 ㄹ -> [ㄴ] (ㄹ의 비음화)
    # ================================================================================
    def test_norm19_words(self):
        """개별 단어 검증: 제19항 본항 (ㅁ, ㅇ 뒤 ㄹ의 비음화)"""
        cases = {
            "담력": "담녁",
            "침략": "침냑",
            "강릉": "강능",
    ...
    def test_norm19_sentences(self):
        """통합 엔진 검증: 제19항 본항 포함 문장"""
        sentence = self._strip_punctuation("담력 하나로 침략을 막아 낸 장수는 강릉에서 항로를 살피던 대통령 곁에 섰다.")
        expected = "담녁 하나로 침냐글 마가 낸 장수는 강능에서 항노를 살피던 대통녕 겨테 섣따"
    ...
    # ================================================================================
    # 제19항 붙임 받침 ㄱ, ㅂ 뒤에 연결되는 ㄹ도 [ㄴ]으로 발음
    #        (선행 받침은 제18항 비음화 연쇄 적용: 막론[막논 -> 망논])
    # ================================================================================
    def test_norm19_addendum_words(self):
        """개별 단어 검증: 제19항 붙임 (ㄱ, ㅂ 뒤 ㄹ의 비음화)"""
        cases = {
            "막론": "망논",
            "석류": "성뉴",
            "협력": "혐녁",
    ...
    def test_norm19_addendum_sentences(self):
        """통합 엔진 검증: 제19항 붙임 포함 문장"""
        sentence = self._strip_punctuation("이유 여하를 막론하고 석류 씨앗을 나누던 두 사람은 협력 끝에 법리 다툼을 피했다.")
        expected = "이유 여하를 망논하고 성뉴 씨아슬 나누던 두 사라믄 혐녁 끄테 범니 다투믈 피핻따"
        actual = apply_phonology(sentence, output_format="hangul")
        g2pk_res = self.g2pk(sentence)
