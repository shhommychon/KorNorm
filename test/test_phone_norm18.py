...
class TestPhoneNorm18(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    # ================================================================================
    # 제18항 받침 ㄱ(ㄲ, ㅋ, ㄳ, ㄺ), ㄷ(ㅅ, ㅆ, ㅈ, ㅊ, ㅌ, ㅎ), ㅂ(ㅍ, ㄼ, ㄿ, ㅄ)
    #        뒤에 ㄴ, ㅁ 결합 -> [ㅇ, ㄴ, ㅁ] (비음화)
    # ================================================================================
    def test_norm18_words(self):
        """개별 단어 검증: 제18항 본항 (비음화)"""
        cases = {
            "먹는": "멍는",
            "국물": "궁물",
            "깎는": "깡는",
    ...
    def test_norm18_sentences(self):
        """통합 엔진 검증: 제18항 본항 포함 문장"""
        sentence = self._strip_punctuation("국물 먹는 아이가 흙만 남은 앞마당에서 꽃망울 맞는 비를 보다가, 옆라인이 반듯한 옷맵시 있는 어른이 짓는 표정을 보고 웃음을 참았다.")
        expected = "궁물 멍는 아이가 흥만 나믄 암마당에서 꼰망울 만는 비를 보다가 염나이니 반드탄 온맵씨 인는 어르니 진는 표정을 보고 우스믈 차맏따"
    ...
    # ================================================================================
    # 제18항 붙임 두 단어를 이어서 한 마디로 발음하는 경우에도 이에 준함
    # ================================================================================
    def test_norm18_addendum_words(self):
        """개별 단어 검증: 제18항 붙임 (두 단어를 이어 발음하는 비음화)"""
        cases = {
            "책 넣는다": "챙 넌는다",
            "흙 말리다": "흥 말리다",
            "옷 맞추다": "온 맏추다",
    ...
    def test_norm18_addendum_sentences(self):
        """통합 엔진 검증: 제18항 붙임 포함 문장"""
        sentence = self._strip_punctuation("가방에 책 넣는다 하고서는 마당에서 흙 말리다, 새 옷 맞추다 말고 밥 먹는다 하며 값 매기다 만 그릇을 챙겼다.")
        expected = "가방에 챙 넌는다 하고서는 마당에서 흥 말리다 새 온 맏추다 말고 밤 멍는다 하며 감 매기다 만 그르슬 챙겯따"
        actual = apply_phonology(sentence, output_format="hangul")
        g2pk_res = self.g2pk(sentence)
