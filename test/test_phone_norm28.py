...
class TestPhoneNorm28(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    # ================================================================================
    # 제28항 표기상 사이시옷이 없더라도 관형격 기능의 사이시옷이 있어야 할 합성어는
    #        뒤 단어의 첫소리 ㄱ, ㄷ, ㅂ, ㅅ, ㅈ을 된소리로 발음
    #        (규칙 일반화 불가 조항 -> 표준국어대사전 발음 선적용 패스가 어휘별로 해결)
    # ================================================================================
    def test_norm28_words(self):
        """개별 단어 검증: 제28항 본항 (사이시옷 없는 관형격 합성어 경음화)"""
        cases = {
            "문고리": "문꼬리",
            "눈동자": "눈똥자",
            "신바람": "신빠람",
    ...
    def test_norm28_sentences(self):
        """통합 엔진 검증: 제28항 본항 포함 문장"""
        sentence = self._strip_punctuation("초승달이 뜬 저녁 길가에 핀 꽃을 보며 아침밥을 먹은 아이는 문고리를 잡고 신바람이 나서 발바닥을 굴렀다.")
        expected = "초승따리 뜬 저녁 길까에 핀 꼬츨 보며 아침빠블 머근 아이는 문꼬리를 잡꼬 신빠라미 나서 발빠다글 굴럳따"
