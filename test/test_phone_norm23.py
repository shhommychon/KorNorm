...
class TestPhoneNorm23(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _word_pipeline(self, word: str) -> str:
        """
        단어 검증용 축약 파이프라인: 엔진 순서(사전 발음 선적용 -> norm20_p -> norm26 -> norm5_p2 -> norm10_p -> LUT -> norm13 -> norm14)를 재현합니다.
    ...
    #        ㄱ, ㄷ, ㅂ, ㅅ, ㅈ은 된소리로 발음 (평파열음 뒤 경음화)
    # ================================================================================
    def test_norm23_words(self):
        """개별 단어 검증: 제23항 본항 (평파열음 뒤 경음화)"""
        cases = {
            "국밥": "국빱",
            "깎다": "깍따",
            "넋받이": "넉빠지",
    ...
    def test_norm23_sentences(self):
        """통합 엔진 검증: 제23항 본항 포함 문장"""
        sentence = self._strip_punctuation("국밥을 먹던 아저씨가 옷고름을 매만지며 꽃다발을 들고 옆집 닭장 앞을 서성였다.")
        expected = "국빠블 먹떤 아저씨가 옫꼬르믈 매만지며 꼳따바를 들고 엽찝 닥짱 아플 서성엳따"
