...
class TestPhoneNorm11(unittest.TestCase):
    ...
    # ============================================================
    # 제11항 ㄺ, ㄻ, ㄿ 은 어말 또는 자음 앞에서 각각 [ㄱ, ㅁ, ㅂ]
    # ============================================================
    def test_norm11_words(self):
        """개별 단어 검증: 제11항 겹받침 대표음 (ㄺ, ㄻ, ㄿ)"""
        cases = {
            "닭": "닥",
            "흙과": "흑꽈",
            "맑다": "막따",
    ...
    # ============================================================
    # 제11항 다만 용언의 어간 말음 'ㄺ'은 'ㄱ' 앞에서 [ㄹ]
    # ============================================================
    def test_norm11_proviso_words(self):
        """개별 단어 검증: 제11항 다만 (용언 어간 ㄺ + ㄱ)"""
        cases = {
            "맑게": "말께",
            "묽고": "물꼬",
            "읽거나": "일꺼나",
    ...
    def test_norm11_proviso_sentences(self):
        """통합 엔진 검증: 제11항 다만 예외 포함 문장"""
        sentence = self._strip_punctuation("날이 맑게 개자, 그는 책을 읽거나 묽고 뜨거운 국물을 마시며 여유를 즐겼다.")
        expected = "나리 말께 개자 그는 채글 일꺼나 물꼬 뜨거운 궁무를 마시며 여유를 즐겯따"
        actual = apply_phonology(sentence, output_format="hangul")
        g2pk_res = self.g2pk(sentence)
